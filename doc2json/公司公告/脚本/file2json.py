...
def f_node_modify(node:gsgg_node):
    """递归文件节点进行子标题的检测和构建

    Args:
        node (gsgg_node): 公司公告的内容节点
    """
    if node is None:
        return
...
def title_list2f_node(file_name:str,title_list:list):
    ...
    """
    f_node = gsgg_node(title = "{}".format(file_name))
    for title in title_list:
        f_node.link(gsgg_node(title=title))
    return f_node
