import re
from gsgg_node import gsgg_node
...
def get_node_content(node, temp_node, toc_mode, log=True):
    ...
    Args:
        node (gsgg_node): 当前节点
        temp_node (gsgg_node): 临时的文件内容容器，用于让节点在匹配后删除已经匹配完的内容，不直接使用str是因为str是不可变变量。
        toc_mode (str): 制表符
    ...
    if node.parent_node is None: # 判断是否是文件节点
        preface_node = gsgg_node(title="preface") # 插入preface节点存储文件第一个标题之前的内容。
...
def get_child_node_content(node,temp,node_list):
    ...
    Args:
        node (gsgg_node): 当前的子标题
        temp (gsgg_node):  临时的文件内容容器，用于让节点在匹配后删除已经匹配完的内容，不直接使用str是因为str是不可变变量。
...
def content_detect(node,content):
        ...
        if index != len(title_list) - 1:
            ...
            if match:
                ...
                if match:
                    ...
                    print("小标题匹配失败：" + title)
    node_list = []
    temp_node = gsgg_node(title=str(node.title)) # 临时节点
    cache = [] # 缓存列表
    if title_list != []:
        for index, i in enumerate(title_list):
            node_list.append(gsgg_node(title=i))
    ...
    if node_list!=[]: # 新增的node数量不为0时
        ...
        temp1 = gsgg_node(title=content) # 临时节点，用于隔开当前检测节点，使得next_node函数正常运作
        for i in node_list:
