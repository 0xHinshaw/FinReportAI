from gptapi import call_llm_api
import re
def sentence_imitation(sentences,chains):
    """根据输入的句子和句子的逻辑链生成一个仿写的句子，以及仿写句子的逻辑链。

    args:
        sentences(list): 输入的句子
        chains(list): 输入的逻辑链
    return:
        tuple : (仿写句子，仿写句子对应的逻辑链)

    """
    prompt_template =  """假设一个金融语言模型能够自动分析用户问题，提取关键内容和逻辑链，并在数据库中匹配相应的句子以生成回答。现有用户问题已匹配到以下句子：{0}，以及上述句子对应的逻辑链：{1}。首先，请识别这些句子涉及的行业和包含的金融要素。然后，基于这些行业信息、金融要素及句子中的逻辑链，重构一个新的句子。这个新句子应整合所有相关内容而不引入额外信息，并确保所包含的逻辑链仅来源于提供的逻辑链。最后，请输出这个句子及其完整的逻辑链，按字典格式输出：{{'sentence':'{{}}', 'chains': {{}}}}。注意：回答须用中文且直接相关，只输出仿写的句子和逻辑链即可，不需要输出行业和金融要素，生成的逻辑链格式为{{"A":,"关系":,"B":}}，不得包含非必要文本。关系必须是列表["并列", "递进", "因果", "转折", "让步", "时间顺序", "条件", "举例", "目的", "假设", "逻辑推理"]中的词，不能是别的词"""
